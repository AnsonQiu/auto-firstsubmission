...
echo "=========================="
echo "Installing App beyondtesla"

# Here is where we perform installation of scripts, services, etc.
echo " - Installing ROS packages for beyondtesla..."
...
runuser -l $LIBPANDA_USER -c /etc/libpanda.d/apps/beyondtesla/installRosPackages.sh

echo "Installing Beyond Tesla..."
...
rosrun robot_upstart install beyondtesla/launch/${LAUNCH_FILE} --user root

echo "Enabling can_to_ros startup script"
sudo systemctl daemon-reload
sudo systemctl enable beyondtesla
popd
echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
