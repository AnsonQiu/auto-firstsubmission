...
// C/C++ source code generated on : Thu Nov 16 12:29:07 2023
//
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"beyondteslaopenloop\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(beyondteslaopenloop_M, (NULL));
        beyondteslaopenloop_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(beyondteslaopenloop_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(beyondteslaopenloop_M)
                == (NULL)) || rtmGetStopRequested(beyondteslaopenloop_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(beyondteslaopenloop_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
